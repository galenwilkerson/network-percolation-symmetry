...
def transform_data_for_plotting(symmetry_data):
    ...
    for z, measures in symmetry_data.items():
        for size in measures['aut_group_sizes']:
            records.append({'Mean Degree (z)': z, 'Measure': 'Size of Automorphism Group', 'Value': size})
        for orbit in measures['orbit_counts']:
            records.append({'Mean Degree (z)': z, 'Measure': 'Number of Orbits', 'Value': orbit})
...
def run_trial_worker(n, z):
    ...
    Parameters:
    n (int): Number of nodes in the graph.
    z (float): Mean degree for the trial.

    Returns:
    tuple: A tuple containing the mean degree, size of the automorphism group, and number of orbits.
    """
    graph = generate_random_graph(n, z)
    aut_group_size, orbit_count = calculate_symmetry_measures(graph)
    gcc_size = len(max(nx.connected_components(graph), key=len)) / n  # Relative size of GCC
    return z, aut_group_size, orbit_count, gcc_size

def run_trials_parallel(n, z_values, trials):
    """
    Runs multiple trials for different z values and computes symmetry measures in parallel.
    ...
    # Prepare a list of arguments for the worker function
    trial_args = [(n, z) for z in z_values for _ in range(trials)]

    # Use ProcessPoolExecutor to run trials in parallel
    results = {z: {'aut_group_sizes': [], 'orbit_counts': [], 'gcc_sizes': []} for z in z_values}
    trial_args = [(n, z) for z in z_values for _ in range(trials)]
    with ProcessPoolExecutor() as executor:
        for z, aut_group_size, orbit_count, gcc_size in executor.map(run_trial_worker, *zip(*trial_args)):
            results[z]['aut_group_sizes'].append(aut_group_size)
            results[z]['orbit_counts'].append(orbit_count)
